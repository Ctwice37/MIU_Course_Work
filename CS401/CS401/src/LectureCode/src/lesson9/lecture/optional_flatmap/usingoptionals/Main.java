...
public class Main {
	...
	private static String telNumberOfBiggestWinnerImperative(List<Company> list) {
		...
		for(Company c: list) {
			if(c.hasWinningTicket()) {
				...
				for(Employee e: emps) {
					int winAmount = e.getWinningAmount();
					if(winAmount > largestSoFar) {
						largestSoFar = winAmount;
						biggestWinner = e;
					}
				}
	...
	private static String telNumberOfBiggestWinner(List<Company> list) {
		return list.stream()
				   ...
				   .findAny()
				              //company -> optional containing winning Employee
				   .flatMap((Company c) -> c.getEmployees().stream()
				       .max(winnings))
				             //employee -> optional containing a tel number
